...
import {ConnectionStatus, ConnInfo, IHubTransport, MessageType} from "./transports/IHubTransport.js";
import type {ThingValue} from "../things/ThingValue.js";
...
// using one of available transport.
export class HubClient {
    ...
    async pubProperties(thingID: string, props: { [key: string]: any }) {
        ...
        if (propsJSON.length > 2) {
            await this.pubEvent(thingID, MessageType.Props, propsJSON);
        }
    }
    ...
    async pubTD(td: ThingTD) {
        ...
        return this.pubEvent(td.id, MessageType.TD, tdJSON);
    }
...
export function NewHubClient(url: string, clientID: string, caCertPem: string, core?: string): HubClient {
    let tp: IHubTransport = new NatsTransport(url, clientID, caCertPem)
