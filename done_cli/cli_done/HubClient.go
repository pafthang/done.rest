...
type HubClient struct {
	...
	clientID string
	//
	transport transport.IHubTransport
	...
	connectionHandler func(status transport.HubTransportStatus)
...
func (hc *HubClient) SplitAddress(addr string) (msgType, agentID, thingID, name string, senderID string, err error) {
	...
	if len(parts) >= 1 && strings.HasPrefix(addr, transport.MessageTypeINBOX) {
		msgType = parts[0]
		if len(parts) >= 2 {
			agentID = parts[1]
		}
...
func (hc *HubClient) GetStatus() transport.HubTransportStatus {
...
func (hc *HubClient) onConnect(status transport.HubTransportStatus) {
	//hc.connectionStatus = status
	//hc.connectionInfo = info
	retryConnect := hc.retryConnect.Load()
	if !retryConnect && status.ConnectionStatus != transport.Connected {
		slog.Warn("disconnecting and not retrying", "clientID", hc.clientID)
		hc.Disconnect()
	} else if status.ConnectionStatus == transport.Connected {
		slog.Warn("connection restored", "clientID", hc.clientID)
	} else if status.ConnectionStatus == transport.Disconnected {
		slog.Warn("disconnected", "clientID", hc.clientID)
	} else if status.ConnectionStatus == transport.Connecting {
...
func (hc *HubClient) onRequest(addr string, payload []byte) (reply []byte, err error, donotreply bool) {
	...
	if agentID != hc.clientID {
		...
		donotreply = true
	} else if messageType == transport.MessageTypeAction && actionHandler != nil {
	...
	} else if messageType == transport.MessageTypeRPC && rpcHandler != nil {
	...
	} else if messageType == transport.MessageTypeConfig && configHandler != nil {
...
func (hc *HubClient) PubAction(
	...
	addr := hc.MakeAddress(transport.MessageTypeAction, agentID, thingID, name, hc.clientID)
...
func (hc *HubClient) PubConfig(
	...
	addr := hc.MakeAddress(transport.MessageTypeConfig, agentID, thingID, propName, hc.clientID)
...
func (hc *HubClient) PubEvent(thingID string, eventID string, payload []byte) error {
	...
	addr := hc.MakeAddress(transport.MessageTypeEvent, hc.clientID, thingID, eventID, hc.clientID)
...
// PubProps publishes a 'properties' (transport.EventNameProps) event containing a map of
...
func (hc *HubClient) PubProps(thingID string, props map[string]string) error {
	...
	addr := hc.MakeAddress(transport.MessageTypeEvent, hc.clientID, thingID,
		transport.EventNameProps, hc.clientID)
...
func (hc *HubClient) PubRPCRequest(
	...
	addr := hc.MakeAddress(
		transport.MessageTypeRPC, agentID, capability, methodName, hc.clientID)
...
func (hc *HubClient) PubTD(td *things.TD) error {
	...
	addr := hc.MakeAddress(
		transport.MessageTypeEvent, hc.clientID, td.ID, transport.EventNameTD, hc.clientID)
...
func (hc *HubClient) SetActionHandler(handler func(msg *things.ThingValue) (reply []byte, err error)) {
	...
	hc.mux.Unlock()
	addr := hc.MakeAddress(
		transport.MessageTypeAction, hc.clientID, "", "", "")
...
func (hc *HubClient) SetConfigHandler(handler func(msg *things.ThingValue) error) {
	...
	hc.mux.Unlock()
	addr := hc.MakeAddress(
		transport.MessageTypeConfig, hc.clientID, "", "", "")
	_ = hc.transport.Subscribe(addr)
	hc.transport.SetRequestHandler(hc.onRequest)
}

// SetConnectionHandler sets the callback for connection status changes.
func (hc *HubClient) SetConnectionHandler(handler func(status transport.HubTransportStatus)) {
	hc.mux.Lock()
...
func (hc *HubClient) SetRPCHandler(handler func(msg *things.ThingValue) (reply []byte, err error)) {
	...
	hc.mux.Unlock()
	addr := hc.MakeAddress(
		transport.MessageTypeRPC, hc.clientID, "", "", "")
	_ = hc.transport.Subscribe(addr)
...
func (hc *HubClient) SubEvents(agentID string, thingID string, eventName string) error {
	...
	subAddr := hc.MakeAddress(
		transport.MessageTypeEvent, agentID, thingID, eventName, "")
...
func NewHubClientFromTransport(transport transport.IHubTransport, clientID string) *HubClient {
	hc := HubClient{
		clientID:  clientID,
		transport: transport,
...
//
// Invoke clidone.ConnectWithXy() to connect
//
...
func NewHubClient(url string, clientID string, caCert *x509.Certificate) *HubClient {
	// a kp is not needed when using connect with token file
	//if kp == nil {
	//	panic("kp is required")
	//}
	var tp transport.IHubTransport = transport.NewNatsTransport(url, clientID, caCert)
