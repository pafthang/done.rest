...
type NatsMsgServer struct {
	Config   *buscfg.NatsServerConfig
	...
	rolePermissions map[string][]modbus.RolePermission
	// map of permissions for each service
	servicePermissions map[string][]modbus.RolePermission
...
//
//	serviceID of the connecting service
//func (srv *NatsMsgServer) ConnectInProc(serviceID string) (*clidone.HubClient, error) {
//
...
//	hc := clidone.NewHubClientFromTransport(tp, serviceID)
//	return hc, err
//}
...
func (srv *NatsMsgServer) Start() (err error) {
	...
	if err != nil {
		...
		subj := transport.MakeSubject(transport.MessageTypeEvent, "", "", "", "")
...
func NewNatsMsgServer(
	cfg *buscfg.NatsServerConfig, rolePermissions map[string][]modbus.RolePermission) *NatsMsgServer {

	srv := &NatsMsgServer{Config: cfg,
		rolePermissions:    rolePermissions,
		servicePermissions: make(map[string][]modbus.RolePermission, 0),
	}
	return srv
}
