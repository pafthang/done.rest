...
//var ServicePermissions = map[string][]modbus.RolePermission{}
...
func (srv *NatsMsgServer) ApplyAuth(clients []modbus.ClientAuthInfo) error {
	...
	authClients := map[string]modbus.ClientAuthInfo{}
...
func (srv *NatsMsgServer) CreateToken(authInfo modbus.ClientAuthInfo) (token string, err error) {
	//
...
func (srv *NatsMsgServer) CreateJWTToken(authInfo modbus.ClientAuthInfo) (newToken string, err error) {
...
func (srv *NatsMsgServer) GetClientAuth(clientID string) (modbus.ClientAuthInfo, error) {
...
func (srv *NatsMsgServer) MakeJWTPermissions(clientInfo modbus.ClientAuthInfo) jwt.Permissions {
...
func (srv *NatsMsgServer) MakePermissions(clientInfo modbus.ClientAuthInfo) *server.Permissions {
	...
	subInbox := transport.MessageTypeINBOX + "." + clientInfo.ClientID + ".>"
	subPerm.Allow = append(subPerm.Allow, subInbox)
	pubInbox := transport.MessageTypeINBOX + ".>"
...
func (srv *NatsMsgServer) SetRolePermissions(
	rolePerms map[string][]modbus.RolePermission) {
...
func (srv *NatsMsgServer) SetServicePermissions(
	...
	for _, role := range roles {
		...
		if rp == nil {
			rp = []modbus.RolePermission{}
		}
		rp = append(rp, modbus.RolePermission{
