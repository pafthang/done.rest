...
func AuthAddUserCommand(hc **clidone.HubClient) *cli.Command {
	displayName := ""
	role := ""
...
func AuthAddServiceCommand(hc **clidone.HubClient, certsDir *string) *cli.Command {
	displayName := ""
...
func AuthListClientsCommand(hc **clidone.HubClient) *cli.Command {
	return &cli.Command{
		Name:     "lu",
		Usage:    "List users",
		Category: "auth",
...
func AuthRemoveClientCommand(hc **clidone.HubClient) *cli.Command {
	return &cli.Command{
		Name:      "rmu",
...
func AuthSetPasswordCommand(hc **clidone.HubClient) *cli.Command {
	return &cli.Command{
		Name:      "setpass",
...
func AuthRoleCommand(hc **clidone.HubClient) *cli.Command {
	return &cli.Command{
		Name:      "setrole",
...
func HandleAddUser(
	hc *clidone.HubClient, loginID string, displayName string, role string) (err error) {

	newPassword := GeneratePassword(9, true)
	authn := authcli.NewManageClients(hc)
...
func HandleAddService(
	hc *clidone.HubClient, serviceID string, displayName string, certsDir string) (err error) {
	var kp keys.IHiveKey
	//TODO: use standardized extensions from launcher
	keyFile := serviceID + ".key"
	authClient := authcli.NewManageClients(hc)
...
func HandleListClients(hc *clidone.HubClient) (err error) {
...
func HandleRemoveClient(hc *clidone.HubClient, clientID string) (err error) {
	authn := authcli.NewManageClients(hc)
...
func HandleSetPassword(hc *clidone.HubClient, loginID string, newPassword string) error {
	if newPassword == "" {
		newPassword = GeneratePassword(9, true)
	}
	authn := authcli.NewManageClients(hc)
...
//	loginID is the ID or email of the user
//	newPassword can be empty to auto-generate a password
func HandleSetRole(hc *clidone.HubClient, loginID string, newRole string) error {
	authn := authcli.NewManageClients(hc)
