...
func DirectoryListCommand(hc **clidone.HubClient) *cli.Command {
	var verbose = false
	return &cli.Command{
		Name:      "ld",
		Category:  "directory",
...
func HandleListDirectory(hc *clidone.HubClient) (err error) {
	offset := 0
	limit := 100
	rdir := dircli.NewReadDirectoryClient(hc)
...
func HandleListThing(hc *clidone.HubClient, pubID, thingID string) error {
	var tdDoc things.TD

	rdir := dircli.NewReadDirectoryClient(hc)
	rhist := histcli.NewReadHistoryClient(hc)
...
func HandleListThingVerbose(hc *clidone.HubClient, pubID, thingID string) error {
	rdir := dircli.NewReadDirectoryClient(hc)
	tv, err := rdir.GetTD(pubID, thingID)
	if err != nil {
		return err
